...
extension OrderDetail {
                    ...
                    if id_order == String(orderDetails!.order!.id_order) {
                        ...
                        repartidorCoords = CLLocationCoordinate2D(
                            latitude: CLLocationDegrees(
                                Double("\(dataString["latitude"]!)")!),
                            longitude: CLLocationDegrees(
                                Double("\(dataString["longitude"]!)")!)
                        )

                        if "\(dataString["status"]!)" != "22" {
                            withAnimation {
                                coords = .region(
                                    MKCoordinateRegion(
                                        center: repartidorCoords!,
                        ...
                        switch "\(dataString["status"]!)" {
                        case "22":
                            statusString = "Recolectando"
                        case "25":
    ...
    func detailOrder() {
        api.fetch(
            url: "orders/details/\(order)", method: "GET", token: user!.token,
            ofType: DetailOrderResponse.self
        ) { res, status in
            if status {
                if res!.status == "success" {
                    ...
                    clientCoords = CLLocationCoordinate2D(
                        latitude: CLLocationDegrees(
                            Double(data.order!.client_latitude)!),
                        longitude: CLLocationDegrees(
                            Double(data.order!.client_longitude)!)
                    )

                    establishmentCoords = CLLocationCoordinate2D(
                        latitude: CLLocationDegrees(
                            Double(data.order!.establishment_latitude)!),
                        longitude: CLLocationDegrees(
                            Double(data.order!.establishment_longitude)!)
                    )
                    ...
                    do {
                        let date = try dateFromString(
                            string: data.order!.created_at)
                        let time = data.order!.time_order.split(separator: ":")
                        var cal = Calendar.current
                        cal.locale = Locale(identifier: "es_MX")
                        var newtime = cal.date(
                            byAdding: .hour, value: Int(time[0])!, to: date!)!
                        newtime = cal.date(
                            byAdding: .minute, value: Int(time[1])!, to: newtime
                        )!
                        fetchRoute(
                            client: clientCoords!,
                            establishment: establishmentCoords!,
                            editDate: newtime)
                    } catch {
    ...
    private func fetchDelivery(_ id: String) {
        api.fetch(
            url: "location/delivery/\(id)", method: "GET",
            ofType: LocationResponse.self
        ) { res, status in
            if status {
                if res!.status == "success" {
                    repartidorCoords = CLLocationCoordinate2D(
                        latitude: CLLocationDegrees(
                            Double(res!.data!.latitude)!),
                        longitude: CLLocationDegrees(
                            Double(res!.data!.longitude)!)
                    )
    ...
    func fetchRoute(
        client: CLLocationCoordinate2D, establishment: CLLocationCoordinate2D,
        editDate: Date
    ) {
        ...
        request.source = MKMapItem(
            placemark: MKPlacemark(coordinate: establishment))
        ...
        Task {
            ...
            var date = calendar.date(
                byAdding: .minute, value: minutes, to: editDate)
